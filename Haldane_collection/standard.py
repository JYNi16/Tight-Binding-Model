...
from ase.build.attach import nearest
...
for k_ind, k in enumerate(k_point_path[:]):
    ...
    H[0, 1] = t_1 * (np.exp(-1j * k @ n1) + np.exp(-1j * k @ n2) + np.exp(-1j * k @ n3))
    H[1, 0] = np.conj(H[0, 1])

    H[0, 0] = t_2 * (e_i_phi * np.exp(-1j * k @ nn1) + e_mi_phi * np.exp(1j * k @ nn1) +
                     e_i_phi * np.exp(-1j * k @ nn2) + e_mi_phi * np.exp(1j * k @ nn2) +
                     e_i_phi * np.exp(-1j * k @ nn3) + e_mi_phi * np.exp(1j * k @ nn3))
    H[1, 1] = t_2 * (e_mi_phi * np.exp(-1j * k @ nn1) + e_i_phi * np.exp(1j * k @ nn1) +
                     e_mi_phi * np.exp(-1j * k @ nn2) + e_i_phi * np.exp(1j * k @ nn2) +
                     e_mi_phi * np.exp(-1j * k @ nn3) + e_i_phi * np.exp(1j * k @ nn3))
