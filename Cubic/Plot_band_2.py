...
def H(k):
    Square_model = Ham.Square_3D(tab=-1, a1=-0.2, b1=-0.3, a2=-0.4, b2=-0.5, a3=-0.0, b3=-0.0)
...
def H2(k):
    Square_model_2 = Ham.Square_3D(tab=-1, a1=-0.5, b1=-0.4, a2=-0.2, b2=-0.1, a3=-0.0, b3=-0.0)
    #ea = np.sort(np.real(np.linalg.eig(model.model_a(k))[0]))
    #eb = np.sort(np.real(np.linalg.eig(model.model_b(k))[0]))
    #e =  np.sort(np.linalg.eig(AFM_s.model(k))[0])
    e =  np.sort(np.linalg.eig(Square_model_2.model(k))[0])
    return e
...
def band_post2():
    ...
    for i in range(len(k_point_path)):
        E_values = np.array(list(map(H2, k_point_path[i])))
        E_band.append(np.real(E_values))
...
def plot_band(): 
    ...
    if len(shape) <= 2:
        ...
        for i in range(shape[-1]):
            ...
            if i == 0:
                plt.plot(k_path, eig, c="red", linewidth=3)
            else:
                plt.plot(k_path, eig, c="green", linewidth=3)
    ...
    plt.subplot(1,2,2)
    if len(shape) <= 2:
        eig2 = np.hstack(tuple(E_band2))
        plt.plot(k_path, eig2, c="red", linewidth=3)
        #plt.xticks(Node,Node_label)
        ...
        for i in range(shape[-1]):
            eig_test2 = [] 
            for j in range(shape[0]):
                eig_test2.append(E_band2[j][:,i])
                print("eig_test.shape is:", len(eig_test2))
            
            eig2 = np.hstack(tuple(eig_test2))
            if i == 0:
                plt.plot(k_path, eig2, c="red", linewidth=3)
            else:
                plt.plot(k_path, eig2, c="green", linewidth=3)
